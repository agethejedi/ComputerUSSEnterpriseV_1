...
function useCanvasInput({ onSubmit, storybookId, episodeId, isActive }) {
  ...
  const submit = async (textNote) => {
    const textParts = [];
    const imageParts = []; // base64 images for Anthropic vision
    ...
    for (const item of items) {
      ...
      if (item.type === "image" && item.preview) {
        // Collect for multimodal API message
        imageParts.push({ name: item.name, dataUrl: item.preview, mimeType: item.mimeType || "image/jpeg" });
      } else {
        textParts.push("[" + item.type.toUpperCase() + ": " + item.name + "]\n" + (item.content || "").slice(0, 500));
      }
    }

    if (textNote && textNote.trim()) textParts.push(textNote.trim());

    if (imageParts.length > 0 || textParts.length > 0) {
      onSubmit({ text: textParts.join("\n\n"), images: imageParts });
    }
    setItems([]);
  };
...
export default function TaniaPanel({ isOpen, onClose }) {
  ...
  const canvas = useCanvasInput({
    onSubmit: ({ text, images }) => sendWithImages(text, images),
  ...
  // Send with optional images — builds multimodal Anthropic content blocks
  const sendWithImages = useCallback(async (text, images = []) => {
    const displayText = images.length > 0
      ? (text ? text + ` [+ ${images.length} image${images.length>1?"s":""}]` : `[${images.length} image${images.length>1?"s":""}]`)
      : text;
    if (!displayText || thinking) return;

    const ronMsg = { role:"ron", content:displayText, id:Date.now() };
    setMessages(prev => [...prev, ronMsg]);

    // Build Anthropic content array — text + image blocks
    const contentBlocks = [];
    for (const img of images) {
      const base64 = img.dataUrl.split(",")[1]; // strip data:image/...;base64,
      contentBlocks.push({
        type: "image",
    ...
    if (text?.trim()) {
      ...
      contentBlocks.push({ type: "text", text: "What do you see here? Tell me what this means for our work." });
    }

    const apiMsg = { role: "user", content: contentBlocks.length === 1 && contentBlocks[0].type === "text"
      ? contentBlocks[0].text  // simple text — keep as string for history
      : contentBlocks           // multimodal — array of blocks
    };

    apiMessagesRef.current = [...apiMessagesRef.current, apiMsg];
    setInput("");
    setThinking(true); setTaniaState("thinking");
    ...
    try {
      ...
      if (responseText) {
        // Store assistant reply as simple text for history (can't send images back)
    ...
    setThinking(false); setTaniaState("resting");
  }, [thinking, activeEpisode, activeStorybook]);
            ...
            <div style={{ flex:1, overflowY:"auto", background:INK2, display:"flex", flexDirection:"column" }}>
              ...
              <div style={{ padding:"0 10px 10px", flex:1, display:"flex", flexDirection:"column", gap:6 }}>
                ...
                <div style={{ fontSize:7, color:"rgba(167,139,250,0.2)", textAlign:"center", letterSpacing:"0.08em" }}>
                  Images → Tania sees them · Files save as artifacts
                </div>
              </div>
            </div>
          )}
