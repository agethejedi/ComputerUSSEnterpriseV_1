...
function loadMusicKit(developerToken) {
  ...
  if (window.MusicKit) {
    try {
      ...
      // getInstance() threw — MusicKit exists but isn't configured yet (State 3)
      // Fall through to configure below
    }
  }

  // A load/configure is already in progress — return the same promise
  ...
  musicKitPromise = new Promise((resolve, reject) => {
    const doConfig = async () => {
      try {
        ...
        musicKitPromise = null; // allow retry on next call
        reject(err);
      }
    };

    // State 3: script already in DOM, MusicKit object exists but not configured
    if (window.MusicKit) {
      doConfig();
      return;
    }

    // State 1: fresh load — inject script
