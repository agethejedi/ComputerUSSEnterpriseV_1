...
// ── loadMusicKit — fixed to handle all three states correctly ─────────────────
// State 1: Fresh page load — script not in DOM yet, MusicKit not configured
// State 2: Script in DOM, configure() ran — getInstance() works
// State 3: Script in DOM, configure() NOT yet run — getInstance() throws
// The original only checked window.MusicKit existence (State 2 vs 1),
// missing State 3 which happens on React StrictMode double-invoke and page
// reloads where the script tag persists but configure() hasn't re-run.
let musicKitPromise = null;

function loadMusicKit(developerToken) {
  // State 2: fully configured — getInstance() works, return immediately
  if (window.MusicKit) {
    try {
      const existing = window.MusicKit.getInstance();
      if (existing) return Promise.resolve(window.MusicKit);
    } catch {
      // getInstance() threw — not configured yet, fall through
    }
  }
  ...
  musicKitPromise = new Promise((resolve, reject) => {
    const doConfig = async () => {
      try {
        // configure() sometimes hangs on MusicKit v3 — race it against a
        // polling fallback that checks every 200ms if getInstance() started
        // working, which means configure completed even if the promise stalled.
        let configSettled = false;

        const configPromise = window.MusicKit.configure({
          developerToken,
          app: { name: "JARVIS Dashboard", build: "1.0.0" },
        }).then(result => { configSettled = true; return result; });

        // Poll for getInstance() success as a fallback signal
        const pollPromise = new Promise((pollResolve, pollReject) => {
          let attempts = 0;
          const poll = setInterval(() => {
            ...
            try {
              ...
              if (inst) {
                clearInterval(poll);
                pollResolve(window.MusicKit);
              }
            } catch {}
            if (attempts > 50) { // 10 seconds of polling
              clearInterval(poll);
              if (!configSettled) pollReject(new Error("MusicKit configure timed out after polling"));
            }
          }, 200);
        });

        // Whichever resolves first wins
        await Promise.race([configPromise, pollPromise]);
    ...
    script.crossOrigin = "";
    script.onload = doConfig;
    script.onerror = () => {
      musicKitPromise = null;
      reject(new Error("MusicKit script load failed"));
    };
      ...
      try {
        ...
        try {
          MusicKit = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("MusicKit load timeout")), 15000)),
          ]);
        } catch (err) {
...
export function IntroOverlay({ introState, songInfo, onSkip, onTap }) {
...
export function useMusicController() {
  ...
  const playSong = useCallback(async (query) => {
    ...
    try {
      ...
      await music.setQueue({ song: song.id });
      await music.play();
      return { ok: true, nowPlaying: { title: song.attributes?.name, artist: song.attributes?.artistName, album: song.attributes?.albumName } };
    } catch (err) {
      return { error: String(err) };
    }
  }, []);
  ...
  const playArtist = useCallback(async (artistName) => {
    ...
    try {
      ...
      const filtered = songs.filter(s => s.attributes?.artistName?.toLowerCase().includes(artistName.toLowerCase()));
      ...
      return { ok: true, count: shuffled.length, nowPlaying: { title: first.attributes?.name, artist: first.attributes?.artistName, album: first.attributes?.albumName } };
    } catch (err) {
      return { error: String(err) };
    }
  }, []);
  ...
  const playAlbum = useCallback(async (albumQuery) => {
    ...
    try {
      ...
      await music.play();
      return { ok: true, nowPlaying: { album: album.attributes?.name, artist: album.attributes?.artistName } };
    } catch (err) {
      return { error: String(err) };
    }
  }, []);

  const pause    = useCallback(async () => { const m = getMusicInstance(); if (!m) return { error: "Apple Music not initialized." }; try { await m.pause(); return { ok: true, state: "paused" }; } catch (e) { return { error: String(e) }; } }, []);
  const resume   = useCallback(async () => { const m = getMusicInstance(); if (!m) return { error: "Apple Music not initialized." }; try { await m.play();  return { ok: true, state: "playing" }; } catch (e) { return { error: String(e) }; } }, []);
  const stop     = useCallback(async () => { const m = getMusicInstance(); if (!m) return { error: "Apple Music not initialized." }; try { await m.stop();  return { ok: true, state: "stopped" }; } catch (e) { return { error: String(e) }; } }, []);
  ...
  const skip = useCallback(async () => {
    ...
    try {
      ...
      return { ok: true, nowPlaying: item ? { title: item.attributes?.name, artist: item.attributes?.artistName } : null };
  ...
  const setVolume = useCallback(async (level) => {
    const music = getMusicInstance();
    if (!music) return { error: "Apple Music not initialized." };
    try { music.volume = Math.max(0, Math.min(1, level / 100)); return { ok: true, volume: level }; }
    catch (err) { return { error: String(err) }; }
  }, []);
  ...
  const nowPlaying = useCallback(() => {
    ...
    return { state: stateLabels[state] || "unknown", nowPlaying: { title: item.attributes?.name, artist: item.attributes?.artistName, album: item.attributes?.albumName, durationMs: item.attributes?.durationInMillis } };
  }, []);

  return { search, playSong, playArtist, playAlbum, pause, resume, skip, previous, setVolume, nowPlaying, stop };
}

// Pause/resume wake word recognition during command listening
export function pauseWakeWords() { wakeWordRegistry.stop(); }
export function resumeWakeWords() { setTimeout(() => wakeWordRegistry.ensureRunning(), 300); }
